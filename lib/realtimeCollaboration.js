  ...
  useEffect(() => {
    ...
    if (!socketRef.current) {
      socketRef.current = io({
        path: '/api/socket',
        auth: { token: "session-token", userId }, // Add authentication data
        reconnectionAttempts: 5,
        reconnectionDelay: 1000,
        timeout: 10000
      });
      socket = socketRef.current; // Update singleton for other hooks
    }
    ...
    return () => {
      if (socket) {
        socket.emit('leave-mindmap', { mindmapId, userId });
        socket.off('connect');
