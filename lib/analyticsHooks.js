...
// Fetch mindmap analytics
export function useMindmapAnalytics() {
  return useQuery({
    queryKey: ['analytics', 'mindmaps'],
    queryFn: async () => {
      const response = await fetch('/api/analytics/mindmaps');
      if (!response.ok) {
        throw new Error('Failed to fetch mindmap analytics');
      }
      return response.json();
    },
  });
